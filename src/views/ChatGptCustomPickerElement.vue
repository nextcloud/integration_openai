<template>
	<div class="chatgpt-picker-content-wrapper">
		<div class="chatgpt-picker-content">
			...
			<div class="footer">
				...
				<NcButton
					type="secondary"
					:aria-label="t('integration_openai', 'Preview text generation')"
				...
				<NcButton v-if="result !== null"
					type="primary"
					:aria-label="t('integration_openai', 'Submit generated text')"
...
export default {
	...
	mounted() {
		...
		if (capabilities.uses_openai) {
			this.poweredByTitle = t('integration_openai', 'by OpenAI')
			this.poweredByUrl = 'https://openai.com'
			this.usesOpenAi = true
		} else {
			this.poweredByTitle = t('integration_openai', 'by LocalAI')
			this.poweredByUrl = 'https://localai.io/features/text-generation/'
			this.usesOpenAi = false
		}
	},
		...
		generate() {
			...
			return axios.post(url, params)
				...
				.catch((error) => {
					...
					showError(
						t('integration_openai', 'Completion request error') + ': '
...
.chatgpt-picker-content-wrapper {
	width: 100%;
}
