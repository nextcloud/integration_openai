...
class OpenAiAPIService
{
	...
	public function request(?string $userId, string $endPoint, array $params = [], string $method = 'GET', ?string $contentType = null): array
	{
		try {
			...
			$this->logger->warning('DEBUUG: Should we use basic authentication with $basicUser: ' . $basicUser . ' and $basicPassword: ' . $basicPassword, ['app' => Application::APP_ID]);
			...
			if (!$this->isUsingOpenAi() && $basicUser !== '' && $basicPassword !== '') {
				$this->logger->warning('DEBUUG: Using basic authentication with $basicUser: ' . $basicUser . ' and $basicPassword: ' . $basicPassword, ['app' => Application::APP_ID]);
