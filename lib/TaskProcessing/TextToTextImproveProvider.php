...
class TextToTextImproveProvider implements IProvider, ISynchronousOptionsAwareProvider {
	...
	public function process(
		...
		if (
			!isset($input['input']) || !is_string($input['input'])
		) {
			throw new ProcessingException('Invalid input');
		}
		$textInput = $input['input'];

		if (!isset($input['instructions']) || !is_string($input['instructions'])) {
			throw new ProcessingException('Invalid instructions');
		}
		...
		foreach ($chunks as $chunk) {
			$prompt = 'Improve the following text based on the provided instructions. Use the same language as the original text. Output only the improved text.'
				. "\n\n*INSTRUCTIONS*:\n$instructions\n\n*TEXT*:\n\n$chunk\n\n"
