...
class TextToSpeechProvider implements ISynchronousProvider {
    ...
    public function getOptionalInputShapeDefaults(): array {
        $adminModel = $this->openAiAPIService->isUsingOpenAi()
            ? ($this->appConfig->getValueString(Application::APP_ID, 'default_speech_voice', Application::DEFAULT_MODEL_ID) ?: Application::DEFAULT_MODEL_ID)
            : $this->appConfig->getValueString(Application::APP_ID, 'default_speech_model_id');
        return [
            'voice' => 'alloy',
            'model' => $adminModel,
        ];
    ...
    public function process(?string $userId, array $input, callable $reportProgress): array {
        ...
        try {
            ...
            return ['speech' => array_pop($apiResponse)];
        } catch (\Exception $e) {
