OC.L10N.register(
    "integration_watsonx",
    {
    "Invalid models response received" : "收到無效的模型回應",
    "Default" : "預設",
    "Text generation" : "產生文字",
    "tokens" : "詞元",
    "Unknown error while retrieving quota usage." : "擷取額度使用量時發生未知錯誤。",
    "Text generation quota exceeded" : "已超過文字產生額度",
    "Unknown text generation error" : "未知的產生文字錯誤",
    "Bad HTTP method" : "錯誤的 HTTP 方法",
    "Bad credentials" : "錯誤的憑證",
    "API request error: " : "API 請求錯誤：",
    "Detect language" : "偵測語言",
    "Friendlier" : "更友善",
    "More formal" : "更正式",
    "Funnier" : "更有趣",
    "More casual" : "更休閒",
    "More urgent" : "更緊急",
    "Maximum output words" : "最大輸出字數",
    "The maximum number of words/tokens that can be generated in the completion." : "補齊中可以產生的最大文字/詞元數。",
    "Model" : "模型",
    "The model used to generate the completion" : "用來產生補齊的模型",
    "Change Tone" : "變更語氣",
    "Ask a question about your data." : "詢問關於您資料的問題。",
    "Input text" : "輸入文字",
    "Write a text that you want the assistant to rewrite in another tone." : "寫一段您希望小幫手以其他語氣重寫的文字。",
    "Desired tone" : "希望的語氣",
    "In which tone should your text be rewritten?" : "您的文字應該以哪種語氣重寫？",
    "Generated response" : "已產生的回應",
    "The rewritten text in the desired tone, written by the assistant:" : "小幫手以您希望的語氣重寫後的文字：",
    "IBM watsonx.ai integration" : "IBM watsonx.ai 整合",
    "Integration of IBM watsonx AI services" : "IBM watsonx AI 服務的整合",
    "⚠ This app is still in early development\nand has only been tested with IBM watsonx.ai as a Service.\n\nThis app implements the following text generation providers\nusing any freely-available large language model:\nFree prompt, Summarize, Headline, Context Write, Chat, and Reformulate.\n\nYou can connect to the watsonx.ai service provided by IBM\nor to a self-hosted cluster running IBM Software Hub and related services.\n\n## Improve AI task pickup speed\n\nTo avoid task processing execution delay, setup at 4 background job workers in the main server (where Nextcloud is installed). The setup process is documented here: https://docs.nextcloud.com/server/latest/admin_manual/ai/overview.html#improve-ai-task-pickup-speed\n\n## Ethical AI Rating\n\n### Rating for Text generation via IBM watsonx.ai: 🟠\n\nPositive:\n* The provided foundation models are freely available, and thus can be ran on-premises\n\nNegative:\n* The software for training and inference of models is proprietary, limiting modifications to the API or other functionality\n* Some foundation models are trained on data that is not freely available, limiting the ability to fine tune them\n\nLearn more about the Nextcloud Ethical AI Rating [in our blog](https://nextcloud.com/blog/nextcloud-ethical-ai-rating/)." : "⚠ 此應用程式仍在早期開發中\n，且僅使用 IBM watsonx.ai 即服務測試過。\n\n本應用程式使用任何免費提供的大型語言模型\n實作下列文字產生提供者：\n自由提示詞、總結、標題、情境寫作、聊天與重新表述。\n\n您可以連線到 IBM 提供的 watsonx.ai 服務\n或執行 IBM Software Hub 及相關服務的自行架設叢集。\n\n## 提昇 AI 任務接手速度\n\n為避免任務處理執行延遲，請在主伺服器（安裝 Nextcloud 的地方）設定 4 個背景作業 workers。設定流程紀錄在此： https://docs.nextcloud.com/server/latest/admin_manual/ai/overview.html#improve-ai-task-pickup-speed\n\n## 道德人工智慧評分\n\n### 透過 IBM watsonx.ai 產生文字的評分：🟠\n\n優點：\n* 提供的基礎模型是免費的，因此可於內部執行\n\n缺點：\n* 模型訓練與推理使用了專有軟體，限制了對 API 或其他功能的修改\n* 部份基礎模型是在無法自由取得的資料上訓練出來的，這限制了微調模型的能力\n\n[在我們的部落格上](https://nextcloud.com/blog/nextcloud-ethical-ai-rating/)取得更多關於 Nextcloud 道德人工智慧評分的資訊。",
    "JSON object. Check the API documentation to get the list of all available parameters. For example: {example}" : "JSON 物件。看看 API 文件以取得所有可用參數清單。例如：{example}",
    "Failed to load models" : "載入模型失敗",
    "Failed to load quota info" : "載入配額資訊失敗",
    "Watsonx.ai admin options saved" : "Watsonx.ai 管理員選項已儲存",
    "Failed to save watsonx.ai admin options" : "儲存 watsonx.ai 管理選項失敗",
    "The Assistant app is not enabled. You need it to use the features provided by the watsonx.ai integration app." : "未啟用小幫手應用程式。您需要它才能使用 watsonx.ai 整合應用程式。",
    "Assistant app" : "小幫手應用程式",
    "Service URL" : "服務 URL",
    "Example: {example}" : "範例：{example}",
    "Leave empty to use {watsonxApiUrl}" : "留空以使用 {watsonxApiUrl}",
    "With the current configuration, the target URL used to get the models is:" : "在目前的組態下，用來取得目標模型的 URL 是：",
    "This should include the domain name and API base path of your watsonx.ai instance. This URL will be accessed by your Nextcloud server." : "這應該包括您的 watsonx.ai 站台的域名與 API 基礎路徑。您的 Nextcloud 伺服器將會存取此 URL。",
    "Service name (optional)" : "服務名稱（選擇性）",
    "Example: watsonx.ai of university ABC" : "範例：ABC 大學的 watsonx.ai",
    "This name will be displayed as provider name in the AI admin settings" : "該名稱將在 AI 管理設定中顯示為提供者名稱",
    "Request timeout (seconds)" : "請求逾時（秒）",
    "Timeout for the request to the external API" : "向外部 API 提出請求的逾時時間",
    "Authentication" : "認證",
    "API key (mandatory with watsonx.ai)" : "API 金鑰（使用 watsonx.ai 時必填）",
    "You can create an API key in your IBM Cloud IAM account settings" : "您可以在您的 IBM Cloud IAM 帳號設定中建立 API 金鑰",
    "Cloud Resources" : "雲端資源",
    "Project ID" : "專案 ID",
    "Space ID" : "空間 ID",
    "A watsonx.ai project ID or space ID is required to generate text" : "產生文字需要 watsonx.ai 專案 ID 或空間 ID",
    "Text completion endpoint" : "文字補完端點",
    "Chat completions" : "聊天補齊",
    "Completions" : "補齊",
    "Using the chat endpoint may improve text generation quality for \"instruction following\" fine-tuned models." : "使用聊天端點可以改善「指令追蹤」微調模型的文字產生品質。",
    "Default completion model to use" : "預設使用的補齊模型",
    "More information about IBM watsonx.ai as a Service" : "更多關於 IBM watsonx.ai 即服務的資訊",
    "More information about the IBM watsonx.ai software" : "更多關於 IBM watsonx.ai 軟體的資訊",
    "Extra completion model parameters" : "額外補完模型參數",
    "Max input tokens per request" : "每個請求的最大輸入詞元數",
    "Split the prompt into chunks with each chunk being no more than the specified number of tokens (0 disables chunking)" : "將指示分割成小區塊，每個區塊不超過指定的詞元數（設定為 0 以停用分割）",
    "Usage limits" : "使用量限制",
    "Quota enforcement time period (days)" : "配額執行期限（天）",
    "Usage quotas per time period" : "每個時段的使用量配額",
    "Quota type" : "配額類型",
    "Per-user quota / period" : "每個使用者的配額/週期",
    "Current system-wide usage / period" : "在目前系統範圍內的使用量/週期",
    "A per-user limit for usage of this API type (0 for unlimited)" : "每個使用者使用此 API 類型的用量限制（0 為無限制）",
    "Max new tokens per request" : "每個請求的最大新詞元數",
    "Maximum number of new tokens generated for a single text generation prompt" : "單一文字產生提示產生的新詞元最大數量",
    "Select enabled features" : "選取已啟用的功能",
    "Text processing providers (to generate text, summarize, context write, etc.)" : "文字處理提供者（用來產生文字、摘要、情境寫作等）",
    "Watsonx.ai options saved" : "Watsonx.ai 選項已儲存",
    "Failed to save watsonx.ai options" : "儲存 watsonx.ai 選項失敗",
    "Your administrator defined a custom service address" : "您的管理員定義了自訂的服務位置",
    "Leave the API key empty to use the one defined by administrators" : "將 API 金鑰留空以使用管理員定義的",
    "API key" : "API 金鑰",
    "A watsonx.ai project ID or space ID is required if an API key is specified" : "若指定 API 金鑰，則需要 watsonx.ai 專案 ID 或空間 ID",
    "Usage quota info" : "使用量配額資訊",
    "Usage" : "使用量",
    "Specifying your own API key will allow unlimited usage" : "指定您自己的 API 金鑰可允許無限制的使用量"
},
"nplurals=1; plural=0;");
